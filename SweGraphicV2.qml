...
Item {
    ...
    function loadSign(j){
        ...
        c+='        let cmd=\'python3 '+unik.currentFolderPath()+'/py/astrologica_swe.py '+vd+' '+vm+' '+va+' '+vh+' '+vmin+' '+vgmt+' '+vlat+' '+vlon+' '+unik.currentFolderPath()+'\'\n'
        c+='    if(apps.showLog){\n'
    ...
    function loadBack(j){
        ...
        c+='    }\n'
        c+='    Component.onCompleted:{\n'
        c+='        let cmd=\'sweg.loadBack() python3 '+unik.currentFolderPath()+'/py/astrologica_swe.py '+vd+' '+vm+' '+va+' '+vh+' '+vmin+' '+vgmt+' '+vlat+' '+vlon+' '+hsys+' '+unik.currentFolderPath()+'\'\n'
        c+='    if(apps.showLog){\n'
        c+='        log.ls(cmd, 0, xApp.width)\n'
        c+='    }\n'
